    ...
    <Card>
      <CardContent className="p-4 text-sm">
          ...
          {Object.entries(planInputs).map(([key, value]) => (
            <div key={key} className="flex flex-col items-center">
              <label htmlFor={key} className="font-bold text-xs mb-2 text-center text-gray-700 bg-gray-100 w-full p-2">
                <strong>
                  {key === 'orderFlow' ? 'Order Flow' :
                   key === 'criticalMass' ? 'Critical Mass' :
                   key === 'currentRange' ? 'Current Range' :
                   key === 'space' ? 'Space' :
                   key === 'tradeType' ? 'Trade Type' :
        ...
        <Textarea
          ...
          className="w-full min-h-[380px] mt-2 resize-none"
